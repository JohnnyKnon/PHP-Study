...
$member = pdo($pdo, $sql, [$id])->fetch();                         // 회원 데이터 가져오기
if (!$member) {                                                    // 배열이 비었다면
    include 'page-not-found.php';                                  // 페이지를 찾을 수 없음
}
...
$articles = pdo($pdo, $sql, [$id])->fetchAll();                     // 회원의 기사
$sql = "SELECT id, name FROM category WHERE navigation = 1;";       // 카테고리를 가져오는 SQL
$navigation   = pdo($pdo, $sql)->fetchAll();                        // 카테고리 가져오기
$section      = '';                                                 // 현재 카테고리
$title        = $member['forename'] . ' ' . $member['surname'];     // HTML <title> 내용
$description  = $title . ' on Creative Folk';                       // 메다 Description
?>
